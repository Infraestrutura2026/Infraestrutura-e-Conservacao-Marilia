...
const App = {
    ...
    renderAlertas() {
        ...
        this.data.Veiculos.forEach(v => {
            if (v.vencSeguro) {
                const venc = new Date(v.vencSeguro + 'T00:00:00');
                const dias = Math.ceil((venc - hoje) / (1000 * 60 * 60 * 24));
                if (dias <= 30 && dias >= 0) {
                    alertas.push({ tipo: 'seguro', texto: `Seguro ${v.placa} vence em ${dias} dias`, cor: dias <= 7 ? 'red' : 'yellow' });
                } else if (dias < 0) {
                    alertas.push({ tipo: 'seguro', texto: `Seguro ${v.placa} VENCIDO`, cor: 'red' });
                }
            }
            ...
            return `
                <div class="veiculo-card" data-id="${v.id}">
                    ...
                    <div class="veiculo-detalhes">
                        ...
                        <div class="veiculo-detalhe"><span>Seguro</span><strong>${formatDateBR(v.vencSeguro)}</strong></div>
    ...
    async saveVeiculo(formData) {
        ...
        const record = {
            ...
            vencSeguro: formData.vencSeguro,
    ...
    editVeiculo(id) {
        ...
        const fields = ['grupo', 'placa', 'marca', 'modelo', 'ano', 'combustivel', 'vencSeguro', 'vencLicenciamento', 'origem', 'status'];
        fields.forEach(f => {
